...
class ABBProgram:
    ...
    def __init__(self):
        ...
        self.HoverPosition      = [380.0, 0.0, 400.0, 180.0, 20.0, 180.0]
        ...
        if Start == "y":
            DIR = os.path.join("TSP_Pictures", "RepeatabilityTestMK2", "%.1fmm" % self.height)
            self.makedir(DIR)
    ...
    def initialize(self):
        ...
        while True:
            try:
                height = float(raw_input("What is the Z value e.g. 390 is just above the Pillow: "))
                if 330 <= height <= 450:
                ...
                with robotLock:
                    ...
                    while ([self.R.get_cartesian()[0], self.r2d(t.euler_from_quaternion(self.R.get_cartesian()[1]))] != euler) and not end.wait(0): # [dataline[0], [round(line,3) for line in dataline[1]]]
                        ...
                        with print_lock:
                            ...
                            print b, euler
                            print [[abs(c1) for c1 in b[0]], [abs(c2) for c2 in b[1]]] in [[abs(e1) for e1 in euler[0]], [abs(e2) for e2 in euler[1]]]
                        if [[abs(c1) for c1 in b[0]], [abs(c2) for c2 in b[1]]] in [[abs(e1) for e1 in euler[0]], [abs(e2) for e2 in euler[1]]]:
                            break
