...
class ABBProgram:
    ...
    def __init__(self):
        ...
        self.HoverPosition                = [320.0, -40.0, 400.0, 180.0, 20.0, 180.0]
        self.movType                      = ["X", "Y", "Z", "Rx", "Ry", "Rz"]
        d                                 = OrderedDict()
        self.error                        = 5
        self.height, self.Row, self.Step, IP, Start = self.initialize()
        self.HoverQuatarnions             = []
        self.HoverPositions               = []
        # Create hover grid
        for j in self.linspace(-40, 40, 10, "+"):
            self.HoverPosition[0] = self.Row
            self.HoverPosition[1] = j
            self.HoverPositions.append(self.HoverPosition[:])
            # Convert self.HoverPosition from euler to quaternion.
            X = copy.deepcopy(self.HoverPosition)
            X = self.d2q(X, 5)
            X = [X[:3], X[3:]]
            self.HoverQuatarnions.append(X[:])

        # Make dictionary.
        [self.get_coordinates(d, self.distance, self.Step, copy.deepcopy(i), _) for _, i in enumerate(self.HoverPositions)]
        dic.put(d)

        if Start == "y":
            DIR = os.path.join("TSP_Pictures", "RepeatabilityTestMK2GRID", "%.1fmm" % self.height, str(self.Row))
            self.makedir(DIR)
    ...
    def initialize(self):
        ...
        while True:
            try:
                height = float(raw_input("What is the Z value e.g. 387.5 is the top of the Pillow: "))
                if 330 <= height <= 450:
        ...
        # Choose the Coordinate that you wish to run.
        while True:
            try:
                Row = float(raw_input("Which row e.g. 320 -> 450 steps of 10: "))
                if 320 <= Row <= 450:
                    print "Accepted - The Robot will perform the run on row %.1f.\n" % Row
                    break
            except:
                pass
            print 'Incorrect input, please try again.\n'
        ...
        while True:
            ...
            if Start in ("y", "n"):
                ...
                print "Incorrect input, please try again.\n"
        return height, Row, step, IP, Start
    ...
    def get_coordinates(self, d, distance, step, touchDownPos, _):
        d[_] = OrderedDict()
        for i in xrange(6):
            XMovement = []
    ...
    def hover(self, Position):
        with robotLock:
            ...
            self.printDateTime()
            print >> self.f, "Heading to Z coordinate %.1fmm" % Position[0][2],
            self.f.flush()
            count = 0
            while (self.R.get_cartesian() != Position):
    ...
    def runMain(self, units, i):
        if not end.wait(0):
            ...
            for _, position in enumerate(self.HoverQuatarnions):
                print position
                self.printDateTime()
                print >> self.f, "Starting Sequence: %s 0%s --> -20%s" % (DIR, units, units)
                self.hover(position)
                data = d[_][self.movType[i]]
