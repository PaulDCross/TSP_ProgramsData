...
class ABBProgram:
    ...
    def __init__(self):
        ...
        self.unit                                       = "mm"
        self.distance                                   = 10
        self.height, self.Step, IP, HH, Tool, self.TYPE = self.initialize()
    ...
    def initialize(self):
        ...
        while True:
            try:
                step = float(raw_input("Choose a step that is in the range of 0.1mm and 10mm: "))
                step = round(step, 1)
                if 0.1 <= step <= 10:
        ...
        while True:
            try:
                TYPE = raw_input("Choose the movment of the sequence, z, roll, pitch:")
                print TYPE
                if TYPE in ('z'):
                    print "Accepted - The Robot will travel {0}{1} along {2}, with a step of {3}{4}.\n".format(self.distance, self.unit, TYPE, step, self.unit)
                    break
                elif TYPE in ('roll', 'pitch'):
                    print "Accepted - The Robot will {0} {1}{2}, with a step of {3}{4}.\n".format(TYPE, self.distance, self.degree, step, self.degree)
                    break
            except:
                pass
            print "Incorrect input, please try again.\n"
    ...
    def runMainTranslation(self, units, i):
        if not end.wait(0):
            with Newtons_lock:
                d = dic.get()
                dic.task_done()
    ...
    def runMainRotation(self, units, i):
        if not end.wait(0):
            with Newtons_lock:
                d = dic.get()
                dic.task_done()
