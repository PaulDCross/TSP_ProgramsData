...
for fold in range(Start, numFolders):
    ...
    for Type in range(len([name for name in os.listdir(directory) if os.path.isdir(os.path.join(directory, name))])):
        ...
        if Record:
            ...
            out    = cv2.VideoWriter(os.path.join(MovementType, 'TSP' + PictureFolder.split("\\")[-3:-1][0] + PictureFolder.split("\\")[-3:-1][1] + "I" + '.avi'),fourcc, 10.0, (1538,656))
            if extrnl:
                # Define the codec and create VideoWriter object
                fourcc = cv2.VideoWriter_fourcc(*'DIVX')
                out2    = cv2.VideoWriter(os.path.join(MovementType, 'TSP' + PictureFolder.split("\\")[-3:-1][0] + PictureFolder.split("\\")[-3:-1][1] + "E" + '.avi'),fourcc, 20.0, (480,480))
...
#############################################################################################
        ...
        for picture in range(first, last):
            ...
            if (SecondImage.all()):
                ...
                DATA                = np.array([tuple(data) for data in [data1[i] + data2[i][1:] + DistanceBearing[i][1:] + [directory[-2:]] + [first] + [picture] + [ls[index][picture][0]] + [ls[index][picture][1]] + [ls[index][picture][2]] + [ls[index][picture][3]] + [ls[index][picture][4]] + [ls[index][picture][5]] + [Types[Type]] + [float(ls[index][picture][2]) - 339.5] for i in xrange(len(keypoints))]], dtype=[('Pin','i4'), ('OriginalXcoord','f4'), ('OriginalYcoord','f4'), ('OriginalPinSize','f4'), ('NewXcoord','f4'), ('NewYcoord','f4'), ('NewPinSize','f4'), ('State','i4'), ('DifferenceX','f4'), ('DifferenceY','f4'), ('Displacement','f4'), ('Bearing','f4'), ('DifferencePinSize','f4'), ('DataSet','i4'), ('PastImage','i4'), ('PresentImage','i4'),('X','f4'), ('Y','f4'), ('Z','f4'), ('Rx','f4'), ('Ry','f4'), ('Rz','f4'), ('Type', 'S4'), ('Depth', 'f4')])
                if A == 0:
                    array1          = DATA
