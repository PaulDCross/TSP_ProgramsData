...
for single in range(1, 2):
    ...
    for set_ in range(10, 11): # for set_ in range(2, Sets):
        ...
        for step in range(1, 2): # for step in range(1, Sets):
            ...
            dataset = SupervisedDataSet(120, 1)
                        ...
                        if (round((zcoordinate - abs(values['Z'][0]))*10, 0)) % step == 0:
                            if 0 < values['DataSet'][0] < set_:
                                if single:
                                    ...
                                    trnEntry = (values[names[Column0]]*10).astype(int)
                                else:
                                ...
                                labelEntry = int(round((zcoordinate - abs(values['Z'][0]))*10, 0))
                            else:
                                if single:
                            ...
                            dataset.addSample(trnEntry, (labelEntry,))
            ...
            fnn = buildNetwork( len(train[0]), 20, 1)
