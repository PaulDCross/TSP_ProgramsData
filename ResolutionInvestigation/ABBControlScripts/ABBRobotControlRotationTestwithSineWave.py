...
class ABBProgram:
    ...
    def __init__(self):
        ...
        self.HoverPosition      = [380.0, 0.0, 390.0, 180.0, 20.0, 180.0]
        ...
        if Start == "y":
            DIR = os.path.join("TSP_Pictures", "RotationTestSineWave", "%.1fmm" % self.height)
            self.makedir(DIR)
    ...
    def get_coordinates(self, d, i, distance, step, touchDownPos):
        ...
        for newValue in self.linspace(touchDownPos[i], touchDownPos[i] + distance, step, "+"):
            ...
            X[2] = round((X[2] / math.sin(math.radians(90-(newValue-touchDownPos[i])))), 1)
